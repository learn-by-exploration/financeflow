...
module.exports = function createApiTokenRoutes({ db, audit }) {
  ...
  router.post('/', (req, res, next) => {
    try {
      ...
      const { name, scope, expires_at } = parsed.data;
      ...
      const token = tokenRepo.create(req.user.id, { name, token_hash, scope, expires_at });
  ...
  // PUT /api/tokens/:id/rotate — rotate a token (invalidate old, issue new)
  router.put('/:id/rotate', (req, res, next) => {
    try {
      const existing = tokenRepo.findById(req.params.id, req.user.id);
      if (!existing) {
        return res.status(404).json({ error: { code: 'NOT_FOUND', message: 'Token not found' } });
      ...
      if (!existing.is_active) {
        return res.status(400).json({ error: { code: 'INVALID_STATE', message: 'Token is already inactive' } });
      }

      const rawToken = 'pfi_' + crypto.randomBytes(32).toString('hex');
      const newHash = crypto.createHash('sha256').update(rawToken).digest('hex');

      const newToken = tokenRepo.rotate(existing.id, req.user.id, newHash);
      audit.log(req.user.id, 'api_token.rotate', 'api_token', existing.id);
      res.json({ token: { ...newToken, raw_token: rawToken } });
    } catch (err) { next(err); }
  });
