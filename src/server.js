...
app.use(helmet({
  ...
  strictTransportSecurity: config.forceHttps ? { maxAge: 31536000, includeSubDomains: true } : false,
...
if (!config.isTest) {
  ...
  function shutdown(signal) {
    ...
    scheduler.stop();
    server.close(() => {
      try { db.pragma('wal_checkpoint(TRUNCATE)'); logger.info('WAL checkpoint completed'); } catch (err) { logger.error({ err }, 'WAL checkpoint failed during shutdown'); }
      db.close();
